...
RELATORIOS_DIR = os.path.join(BASE_DIR, "saidas", "relatorios")
...
def verificar_e_gerar(df: pd.DataFrame) -> str:
    """
    Chamado pelo watcher todo domingo.
    Gera o PDF da semana que acabou (domingo anterior a sabado ontem).
    Retorna caminho do PDF ou string vazia se nao for domingo.
    """
    hoje = date.today()

    # So executa aos domingos (isoweekday == 7)
    if hoje.isoweekday() != 7:
        return ""

    inicio, fim = intervalo_semana_anterior()

    # Verificar se ja foi gerado hoje
